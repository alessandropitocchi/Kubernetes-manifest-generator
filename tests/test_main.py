...
def test_generate_ingress():
    ingress = generate_ingress("test-ingress", "default", "example.com", "/", "test-service", 80)
    ...
    assert ingress["spec"]["rules"][0]["http"]["paths"][0]["backend"]["service"]["name"] == "test-service"

# Test per il salvataggio del manifest
def test_save_manifest(tmpdir):
    manifest_data = {"apiVersion": "v1", "kind": "Pod", "metadata": {"name": "test-pod"}}
